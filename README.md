...
This project needs the ACDC dataset to run, mainly for quantification_methods.py and for both neural network folders (xarxaResnet18 and xarxaentrenadadesde0), since they load the raw images directly. The dataset is not included in this repository, so you need to download it yourself from the official ACDC page and place it inside a folder named data/ACDC/ at the root of the project, keeping the same training/testing split and per-patient folders as they come in the original download. If your local copy is stored somewhere else, just update the data path in config.py of the corresponding module.
...
Usage: run the generator script to create the training/testing CSV files before training any model.

# analisisfeaturesmanuals

Analysis of geometric features extracted manually from the images, and their quantification using classical Machine Learning models.
quantification_methods.py: quantification methods for the manual descriptors/features.
modelsmlfeaturesgeometriques.py: defines and trains ML models (non-neural) on the extracted geometric features.
...
Univariate correlation analysis between the features and the target variables, split by cardiac phase (ED/ES) and aggregation level (per patient / per slice).
analisiscorrelaciounivariada.py: main script; computes the correlation matrices, generates the heatmaps and extracts the top correlations.
...
Multivariate correlation analysis (multiple regression, R2) between sets of features and the target variables.
analisiscorrelaciomultivariada.py: main script; fits the multivariate models and computes the R2 coefficient.
...
CNN based on the ResNet18 architecture (transfer learning / feature extraction), used for image feature extraction and biomarker prediction.
...
CNN trained from scratch (without transfer learning), as a comparison against the ResNet18 approach.
config.py: model parameters (hyperparameters, paths, etc.).
...
Note: config.py and train_utils.py are the same files in both network folders (xarxaResnet18 and xarxaentrenadadesde0); they are duplicated because their contents are shared by both models.
